...
def compute_pixelwise_retrieval_metrics(anomaly_segmentations, ground_truth_masks):
    ...
    segmentations_bin = (anomaly_segmentations >= optimal_threshold).astype(np.uint8)
    masks_gt_bin = (ground_truth_masks > 0).astype(np.uint8)
    

    pros = []
    for seg, gt in zip(segmentations_bin, masks_gt_bin):
        intersection = np.sum(seg & gt)
        union = np.sum(seg | gt)         
        if union == 0: 
            pros.append(1.0 if np.sum(gt) == 0 else 0.0)
        else:
            pros.append(intersection / union)
    pro = np.mean(pros)
